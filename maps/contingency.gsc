...
//	maps\_remotemissile::init_radio_dialogue();
	...
	//New 06.03.2022 (2:07 PM)
	flag_init( "friendlies_or_player_at_sub" );
	flag_init( "obj_sub_entrance" );
	flag_init( "player_on_sub" );
	flag_init( "player_dropping_into_sub" );
	flag_init( "player_is_clear_of_ladder" );
	flag_init( "tear_gas_guys_dead" );
	flag_init( "soap_truck_arrived" );
	flag_init( "player_in_uaz" );
	...
	//Added 06.03.2023 (3:04 PM)
	triggers_hurt = getEntArray("trigger_hurt", "classname");
	for( i = 0; i < triggers_hurt.size; i++)
	{
		if(isDefined(triggers_hurt[i].origin) && triggers_hurt[i].origin == (-9428, 2078, 334)) // -9428 2078 334
		{
			triggers_hurt[i] delete();
		}
	}
...
handle_base()
{
	...
	//thread handle_defend_sub();
	/#
	iPrintLnBold("Sub Gameplay Init");
	#/
	thread handle_sub();
}
...
start_sub()
{
	...
	//Added 06.03.2023 (9:08 PM)
	thread objective_defend_sub();
...
handle_sub()
{
	...
	killTimer();
	level.uav_radio_disabled = true;
	level.uav_is_not_available = true;
	level.player maps\_remotemissile::disable_uav( false, true );

	//Added 07.03.2023 (8:25 PM)
	level notify ("timer_killed");

	thread dialog_at_sub();
	
	flag_set( "obj_sub_entrance" );//player at sub
	...
	//Added 06.03.2023 (3:34 PM)
	radio = spawn( "script_origin", (-9638, 2112, 346) );

	//Added 07.03.2023 (5:51 PM)
	autosave_by_name( "sub_defend_start" );

	//Roach! I need a few minutes! Cover me!	
	level.price dialogue_queue( "cont_pri_needfewminutes" );
	
	//Alert! The use of nuclear weapons has been authorized.
	radio playsound("cont_rsc_alert", "sounddone");
	...
	//Launch codes verified authentic. Launch codes accepted.
	radio playsound("cont_rsc_launchcodes", "sounddone");
	radio waittill("sounddone");
	wait randomFloatRange(3, 5);
	//Set condition one-SQ. Nuclear missile launch authorized.	
	radio playsound("cont_rsc_setcondition", "sounddone");
	radio waittill("sounddone");
	wait randomFloatRange(3, 5);
	//Now spinning up missiles 1 through 4 and 8 through 12 for strategic missile launch
	radio playsound("cont_rsc_spinningup", "sounddone");
	radio waittill("sounddone");
	wait randomFloatRange(4, 6);
	//Target package 572 has been authorized. Standing by for target confirmation and launch order.
	//radio playsound("cont_rsc_launchorder", "sounddone");
	//radio waittill("sounddone");
	//wait randomFloatRange(3, 5);
	
	//Added 07.03.2023 (5:52 PM)
	autosave_by_name( "sub_defend_wave2" );
	...
	radio playsound("cont_rsc_missilesready", "sounddone");
	radio waittill("sounddone");
...
handle_exit_sub()
{
	...
	//Added 07.03.2023 (2:15 PM)
	level.soap_truck godon();
	
	//level.price forceteleport( price_wait_for_truck_node.origin );
	//We're done here! Let's go!	
	level.price thread dialogue_queue( "cont_pri_donehereletsgo" );

	//Added 07.03.2023 (5:53 PM)
	autosave_by_name( "sub_defend_completed" );
	...
	price_on_sub = getnode( "price_on_sub", "targetname" );
	level.price forceteleport( price_on_sub.origin );
	...
	//iPrintLnBold("mission ended");
	nextmission();
}

//Added 07.03.2023 (2:37 PM)
setup_soap()
{
...
setup_sub_hatch()
{
	...
	hatch_model_collision connectPaths();
...
spawn_sub_enemies()
{
	...
	array_thread( sub_enemies, ::add_spawn_function, ::ai_sub_think );
	while( 1 )
	{
...
base_alarm_sound()
{
	...
	wait 1;
	base_pa playsound( "cont_bpa_underattack_sub" );
	wait 12;
	...
	base_alarm_sound StopLoopSound();
	base_alarm_sound.playing = undefined;
	base_pa playsound( "cont_bpa_prejudice_sub" );
	wait 12;
	...
	base_alarm_sound StopLoopSound();
	base_alarm_sound.playing = undefined;
	base_pa playsound( "cont_bpa_2ndplatoon_sub" );
	wait 12;
	...
	base_alarm_sound StopLoopSound();
	base_alarm_sound.playing = undefined;
	base_pa playsound( "cont_bpa_alert_sub" );
	wait 12;
	...
	base_alarm_sound StopLoopSound();
	base_alarm_sound.playing = undefined;
	base_pa playsound( "cont_bpa_battlestations_sub" );
	wait 12;
